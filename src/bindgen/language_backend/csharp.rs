...
impl LanguageBackend for CSharpLanguageBackend<'_> {
    fn open_namespaces<W: Write>(&mut self, out: &mut SourceWriter<W>) {
        ...
        if let Some(extra) = &self.config.csharp.extra_defs {
            write!(out, "{extra}");
            out.new_line();
        }
    }
    ...
    fn write_headers<W: Write>(&self, out: &mut SourceWriter<W>, package_version: &str) {
        ...
        if let Some(ref package) = self.config.csharp.package {
            out.new_line_if_not_start();
...
pub(crate) fn wrap_csharp_value(literal: &Literal, ty: &Type) -> Literal {
    match literal {
...
pub(crate) fn csharp_writable_literal(ty: &Type, literal: &Literal) -> bool {
    // quite crude for now
    match literal {
        Literal::Expr(e) => {
